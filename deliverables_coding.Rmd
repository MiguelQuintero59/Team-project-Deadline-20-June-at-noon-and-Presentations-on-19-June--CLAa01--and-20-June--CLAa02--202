...
best_single <- best_results %>%
  ...
  pivot_longer(
    cols = -c(Model, resampling, nfeatures, Genes),
  ...
  relocate(Model, resampling, Subset, Accuracy, Sensitivity, Specificity, PPV, NPV, AUC, nfeatures, Genes)
...
# Confusion Matrix
cm <- confusionMatrix(predictions, true_labels, positive = "X1")
...
metrics_rf_train_fs <- data.frame(
  Model = "RF",
  resampling = 'No RS',
  Subset = 'Train',
  Accuracy = as.numeric(cm$overall["Accuracy"]),
...
# Model: SVM Full Variables
# Resample Technique: K- Fold Cross Validation
```{r setup, include=FALSE}
set.seed(125) 
...
# Estimate sigma for radial kernel using full data
sig_est    <- kernlab::sigest(Class ~ ., data = df_train)
sigma_med  <- as.numeric(sig_est)[2]

# Define grid of sigma and cost
svm_grid_full <- expand.grid(
      sigma = sigma_med * c(0.5, 1, 2),
      C     = c(0.25, 1, 4)
    )
...
metrics_svm_train_cv <- data.frame(
  Model = "SVM",
  resampling = 'CV',
  Subset = 'Train',
  Accuracy = as.numeric(cm$overall["Accuracy"]),
...
# === Testing Model Stats ===
prob_test <- predict(model_sv, newdata = df_test, type = "prob")
pred_class_test <- predict(model_sv, newdata = df_test, type = "raw")
...
cm_test <- confusionMatrix(
  pred_class_test,
  reference = df_test$Class, 
  positive = "X1"
  )

# AUC
roc_svm <- pROC::roc(response = as.factor(df_test$Class),
               predictor = prob_test$X1,
               levels = rev(levels(as.factor(df_test$Class))))
auc_value_svm <- auc(roc_svm)

metrics_svm_test_cv <- data.frame(
  Model = "SVM",
  resampling = 'CV',
  Subset = 'Test',
  Accuracy = as.numeric(cm_test$overall["Accuracy"]),
  ...
  NPV = as.numeric(cm_test$byClass["Neg Pred Value"]),
  AUC = as.numeric(auc_value_svm),
  Nfeats = as.numeric(ncol(df_test) - 1)
)
...
metrics_svm_train_cv_fs <- data.frame(
  Model = "SVM",
  resampling = 'CV',
  Subset = 'Train',
  Accuracy = as.numeric(cm$overall["Accuracy"]),
...
metrics_svm_test_cv_fs <- data.frame(
  Model = "SVM",
  resampling = 'CV',
  Subset = 'Test',
  Accuracy = as.numeric(cm_test$overall["Accuracy"]),
...
# Load Data filter using feature selection from first point 
df <- load_gene_subset(group_number = 7, include_class = TRUE)
df$Class <- factor(df$Class, levels = c(1, 2), labels = c("X1","X2"))
...
svm_grid <- expand.grid(
  sigma = sigma_med,  # middle estimate
  C     = 1           # chosen cost
)
...
# === Training Model Stats === 
# Predict probabilities & classes on the training set
train_probs  <- predict(model_svm, df_train, type = "prob")[, "X1"]
train_preds  <- predict(model_svm, df_train)
train_preds <- factor(train_preds, levels = levels(df_train$Class))

roc_obj <- pROC::roc(
  response  = df_train$Class,
  predictor = train_probs,  
  levels    = rev(levels(df_train$Class))
)

auc_value_svm <- auc(roc_obj)

# Get predicted classes and true labels
predictions <- model_svm$pred$pred
true_labels <- model_svm$pred$obs

# Confusion Matrix
cm <- confusionMatrix(train_preds, df_train$Class,, positive = "X1")
...
metrics_svm_train_fs <- data.frame(
  Model = "SVM",
  resampling = 'No RS',
  Subset = 'Train',
  Accuracy = as.numeric(cm$overall["Accuracy"]),
...
metrics_svm_test_fs <- data.frame(
  Model = "SVM",
  resampling = 'No RS',
  Subset = 'Test',
  Accuracy = as.numeric(cm_test$overall["Accuracy"]),
...
metrics_svm <- rbind(
  metrics_svm_train_cv,
  metrics_svm_test_cv,
  metrics_svm_train_cv_fs,
  metrics_svm_test_cv_fs,
  metrics_svm_train,
  metrics_svm_test,
  metrics_svm_train_fs,
  metrics_svm_test_fs)
...
metrics_unique <- metrics[!duplicated(metrics), ]
```
