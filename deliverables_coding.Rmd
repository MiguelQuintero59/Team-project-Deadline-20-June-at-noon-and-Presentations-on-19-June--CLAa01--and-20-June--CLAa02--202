...
sensitivity_svm <- cm$byClass["Sensitivity"]
specificity_svm <- cm$byClass["Specificity"]
ppv_svm <- cm$byClass["Pos Pred Value"]
npv_svm <- cm$byClass["Neg Pred Value"]
nfeats_svm <- ncol(train_df) - 1 
...
metrics_svm <- data.frame(
  Model = "SVM",
  Accuracy = as.numeric(accuracy_svm),
  Sensitivity = as.numeric(sensitivity_svm),
  Specificity = as.numeric(specificity_svm),
  PPV = as.numeric(ppv_svm),
  NPV = as.numeric(npv_svm),
  AUC = as.numeric(auc_value_svm),
  Nfeats = as.numeric(nfeats_svm)
)
...
metrics_all <- rbind(metrics_rf, metrics_lda,metrics_svm)
...
rownames(radar_data) <- c("Max", "Min", metrics_all$Model)
...
radarchart(
  ...
  cglwd = 0.8,
  vlcex = 0.8,
  title = "Normalized Performance Comparison (RF vs LDA)"
)

legend(
  x = "topright", legend = metrics_all$Model,
  bty = "n", pch = 20, col = colors_border,
  text.col = "grey40", cex = 0.9, pt.cex = 2
)
...
# Normalize function: handles constant columns gracefully
normalize_column <- function(x) {
  rng <- range(x, na.rm = TRUE)
  if (diff(rng) == 0) {
    return(rep(0.5, length(x)))  # If constant, return 0.5
  } else {
    return((x - min(x)) / diff(rng))  # Normalize to 0-1
  }
}

# Apply normalization (excluding 'Model')
metrics_normalized <- metrics_all %>%
  mutate(across(where(is.numeric) & !all_of("Model"), normalize_column))

# === Plot Each Model Individually ===
# Define colors
colors_border <- c("blue", "red", "darkgreen")
colors_in <- adjustcolor(colors_border, alpha.f = 0.3)

# Set layout: one plot per model
par(mfrow = c(1, 3), mar = c(2, 2, 2, 2))

for (i in 1:nrow(metrics_normalized)) {
  model_name <- metrics_normalized$Model[i]
  values <- metrics_normalized[i, -which(names(metrics_normalized) == "Model")]
  ...
  rownames(radar_data) <- c("Max", "Min", model_name)
