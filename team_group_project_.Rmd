...
load_gene_subset <- function(group_number,
                             expression_file = "/Users/macbook/Downloads/Team project Deadline 20 June at noon and Presentations on 19 June (CLAa01) and 20 June (CLAa02)-202/gene-expression-invasive-vs-noninvasive-cancer.csv",
                             subset_file = "/Users/macbook/Downloads/Team project Deadline 20 June at noon and Presentations on 19 June (CLAa01) and 20 June (CLAa02)-202/teamsubsets.csv",
                             include_class = TRUE) {
  library(dplyr)
  library(caret)
  library(glmnet)
...
# Reload original data with class
df_full <- load_gene_subset(group_number = 7, include_class = TRUE)

# Add patient cluster labels as a new feature
df_with_clusters <- df_full
df_with_clusters$Cluster <- as.factor(patient_clusters)

# LDA with cluster as extra feature
lda_model_cluster <- lda(Class ~ . + Cluster, data = df_with_clusters)

# Predict on training data
predictions <- predict(lda_model_cluster, newdata = df_with_clusters)

# Confusion matrix
cm <- confusionMatrix(as.factor(predictions$class), as.factor(df_with_clusters$Class), positive = "1")
print(cm)
